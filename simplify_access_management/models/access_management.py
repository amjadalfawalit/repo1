...
class access_management(models.Model):
    ...
    hide_menu_ids = fields.Many2many('ir.ui.menu', 'access_management_menu_rel_ah', 'access_management_id', 'menu_id', 'Hide Menu',copy=True)
    ...
    def get_remove_options(self, model):
        remove_action = self.env['remove.action'].sudo().search([('access_management_id','in',self.env.user.access_management_ids.ids),('model_id.model','=',model)])
        options = []
