odoo.define("advanced_web_domain_widget.TerabitsDomainSelector", function (require) {
    "use strict";
     ...
     */
    var TerabitsDomainNode = Widget.extend({
        events: {
        ...
        /**
         * A TerabitsDomainNode needs a model and domain to work. It can also receive a set
         * of options.
         *
         * @param {Object} parent
    ...
    /**
     * TerabitsDomainNode which can handle subdomains (a domain which is composed of
     * multiple parts). It thus will be composed of other TerabitsDomainTree instances
     * and/or leaf parts of a domain (@see DomainLeaf).
     */
    var TerabitsDomainTree = TerabitsDomainNode.extend({
        template: "TerabitsDomainTree",
        events: _.extend({}, TerabitsDomainNode.prototype.events, {
        ...
        /**
         * @constructor
         * @see TerabitsDomainNode.init
         * The initialization of a TerabitsDomainTree creates a "children" array attribute
         * which will contain the the TerabitsDomainNode children. It also deduces the
         * operator from the domain.
         * @see TerabitsDomainTree._addFlattenedChildren
         */
        ...
        /**
         * @see TerabitsDomainNode.isValid
         * @returns {boolean}
         */
        isValid: function () {
        ...
        /**
         * @see TerabitsDomainNode.getDomain
         * @returns {Array}
         */
        getDomain: function () {
            var childDomains = [];
         ...
         *                       widget
         * @param {TerabitsDomainNode} afterNode - the node after which the new domain part
         ...
         *
         * @private
         * @param {TerabitsDomainNode} oldChild - the child instance to remove
        ...
        _parseDomain: function (domain) {
            ...
            try {
                ...
                // currently not handled by the TerabitsDomainSelector
     ...
     */
    var TerabitsDomainSelector = TerabitsDomainTree.extend({
        ...
        /**
         * If 'TerabitsDomainSelector' is attached within modal, the modal should have
        ...
        _onDebugInputChange: function (e) {
            ...
            var rawDomain = e.currentTarget.value;
            try {
                rawDomain = Domain.prototype.stringToArray(rawDomain);
            ...
            this._redraw(rawDomain).then((function () {
                this.trigger_up("domain_changed", { child: this});
            }).bind(this));
        },
        /**
     ...
     */
    var TerabitsDomainLeaf = TerabitsDomainNode.extend({
        template: "TerabitsDomainLeaf",
        events: _.extend({}, TerabitsDomainNode.prototype.events, {
            ...
            "click .o_record_selector_item": "on_add_record_tag",
            ...
            defs.push(this.fieldSelector.appendTo($("<div/>")).then((function () {
                ...
                if (!this.readonly) {
                    ...
                    if (selectedField.type === "boolean") {
                        ...
                        // To read record of relational field which is added in domain.
        ...
        start: function () {
            ...
            var selectedField = this.fieldSelector.getSelectedField() || {}
            if ((_.contains(['many2one', 'many2many', 'one2many'], selectedField.type)) && (_.contains(['in', 'not in'], this.operator))) {
                // this.recordReader.$el.appendTo(this.$("> .o_domain_leaf_info, > .o_domain_leaf_edition")); // place the field selector
                 
                this.$('.o_domain_leaf_value_tags').replaceWith(this.recordReader.$el);
                // this.displayValue = []
            }
        ...
        _changeValue: function (value, silent) {
            ...
            } else if ((_.contains(['many2many', 'one2many', 'many2one'], selectedField.type)) && (_.contains(['in', 'not in'], this.operator))) {
        ...
        on_add_record_tag: function (e) {
            // e.preventDefault();
            // e.stopPropagation();this.tagsvalues = []
            if (!_.contains(["not in", "in"], this.operator)) return;
            var values = _.isArray(this.value) ? this.value.slice() : [];
            // var tags = _.isArray(this.tagsvalues) ? this.tagsvalues.slice() : [];

            // var $input = this.$(".o_domain_leaf_value_tags input");
             
            var val = $(e.target).data('id');
            
            if ((val || val == 0 )&& values.indexOf(val) < 0) {
                values.push(val);
                // this.tagsvalues.push(tag);
                _.defer(this._changeValue.bind(this, values));
            }
        },
        on_remove_record_tag: function (e) {
            var values = _.isArray(this.value) ? this.value.slice() : [];
    ...
    /**
     * Instantiates a TerabitsDomainTree if the given domain contains several parts and a
     ...
     * @returns {TerabitsDomainTree|DomainLeaf|null}
     */
    function instantiateNode(parent, model, domain, options) {
        if (domain.length > 1) {
            return new TerabitsDomainTree(parent, model, domain, options);
        } else if (domain.length === 1) {
            return new TerabitsDomainLeaf(parent, model, domain, options);
        }
        return null
    }
    return {
